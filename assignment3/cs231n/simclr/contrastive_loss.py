...
def sim(z_i, z_j):
    ...
    zi_norm = torch.linalg.norm(z_i)
    zj_norm = torch.linalg.norm(z_j)
    norm_dot_product = z_i.dot(z_j) / (zi_norm * zj_norm)
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        nu = torch.exp(sim(z_k, z_k_N) / tau)
        de_left, de_right = 0, 0
        for i in range(2*N):
            if i != k:
                de_left += torch.exp(sim(z_k, out[i]) / tau)
            if i != k+N:
                de_right += torch.exp(sim(z_k_N, out[i]) / tau)
        loss_left = -torch.log(nu / de_left)
        loss_right = -torch.log(nu / de_right)
...
def sim_positive_pairs(out_left, out_right):
    ...
    norm_left = torch.linalg.norm(out_left, dim=1, keepdims=True)  # (N,1)
    norm_right = torch.linalg.norm(out_right, dim=1, keepdims=True)  # (N,1)
    MM = out_left.matmul(out_right.T)  # (N,N)
    inner_prod = torch.diag(MM, 0).reshape(-1, 1)  # (N,1)
    pos_pairs = inner_prod / (norm_left * norm_right)
...
def compute_sim_matrix(out):
    ...
    norm_vector = torch.linalg.norm(out, dim=1)  # (2N,)
    normalized_out = out / norm_vector.reshape(-1, 1)
    sim_matrix = normalized_out.matmul(normalized_out.T)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    spp = sim_positive_pairs(out_left, out_right)
    ...
    tot_spp = torch.cat((spp, spp))
    numerator = torch.exp(tot_spp / tau).to(device)
